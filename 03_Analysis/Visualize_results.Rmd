...
# To compare community profiles across treatments, the PERMANOVA method was used. To do this, we used Vegan (v2.6-4) package to obtain Bray-Curtis distances from the log-transformed sample vs. bacterial abundance matrix. The distance matrix was then used to make a PCoA and the adonis2 function was used to perform the PERMANOVA analysis. The effect size (omega squared) was calculated using the adonis_OmegaSq function from the RVAideMemoire package (v0.9-81). The results are shown in the PCoA plots as omega squared and p-value.
...
pvals_non_zero <- df_species_count_plot_diff %>%
  filter(!arrival %in% c("dropout")) %>%
  group_by(Strain) %>%
  filter(abs_counts > 1) %>%
  wilcox_test(abs_counts ~ arrival, data = .) %>%
...
fig_strain_order <- c("ESL0820", "ESL0825", "ESL0170", "ESL0822", "ESL0198",
                      "ESL0824", "ESL0199", "ESL0827", "ESL0200", "ESL0819",
                      "ESL0184", "ESL0350", "ESL0394", "ESL0185", "ESL0263",
                      "ESL0183", "ESL0835", "ESL0186", "ESL0261", "ESL0295",
                      "ESL0835")

write.csv(pvals_non_zero %>%
            filter(group1 != "only") %>%
            filter(group2 != "only") %>%
            mutate(Strain = factor(Strain, levels = unique(fig_strain_order))) %>%
            arrange(Strain),
          "03_Analysis/Results/tables/priority_effect_strength/fig3_beeswarm_pvals.csv", row.names = F, quote = F)
...
ggplot() +
  ...
  geom_signif(data = pvals_non_zero, manual = TRUE,
...
ggsave("03_Analysis/Results/figures/colonization_success/counts_by_arrival_per_strain_nonzero_p.pdf", width = 10, height = 15)


df_species_count_fischers <- df_species_count_all_combined %>%
                                filter(Experiment %in% c("1", "2")) %>%
                                filter(Type == "bee") %>%
                                filter(arrival %in% c("first", "second", "only")) %>%
                                mutate(fisch_input = ifelse(abs_counts < 1, 0, 1)) %>%
                                select(Strain, arrival, fisch_input, ID) %>%
                                group_by(Strain, arrival) %>%
                                summarize(present = sum(fisch_input),
                                          absent = n() - sum(fisch_input)) %>%
                                pivot_wider(names_from = arrival, values_from = c(present, absent)) %>%
                                rowwise() %>%
                                mutate(fishers_pval = fisher.test(matrix(c(present_first, absent_first,
                                                                           present_second, absent_second),
                                                                         nrow = 2))$p.value) %>%
                                ungroup() %>%
                                mutate(fishers_pval_adj = p.adjust(fishers_pval, method = "BH")) %>%
                                mutate(fishers_sig = ifelse(fishers_pval_adj < 0.05, "*", "ns.")) %>%
                                mutate(fishers_sig = ifelse(fishers_pval_adj < 0.01, "**", fishers_sig)) %>%
                                mutate(fishers_sig = ifelse(fishers_pval_adj < 0.001, "***", fishers_sig))
write.csv(df_species_count_fischers, "03_Analysis/Results/tables/priority_effect_strength/fig3_fishers_pvals.csv", row.names = F)
...
meds <- df_comparison_full_drop_mod %>%
  ...
  mutate(
    ...
    med_full_sec = med[dropout_type == "Full (second)"][1],
    pct_vs_full_first = (med - med_full_sec) / (med_full_first - med_full_sec) * 100,
  ...
  mutate(
    ...
    pct_lab = ifelse(dropout_type == "Full (first)", "100%", sprintf("%.2f%%", pct_vs_full_first)),
...
```
glmer(abs_counts_trans ~ arrival + (1 | CommunityCombo), 
                 family = Gamma(link = "log"),
                 data = dat)

For the generalized linear mixed model (GLMM), we can use the `glmer` function from the `lme4` package in R. The model will predict the transformed absolute counts (`abs_counts_trans`) based on the arrival order (`arrival`), while accounting for random effects due to different community combinations (`CommunityCombo`). 
It was carried out using a Gamma distribution with a log link function, which is appropriate for modeling positive continuous data like counts.